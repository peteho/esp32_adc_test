...
void app_main(void)
{
    ...
    uint32_t v;
    esp_err_t r;
    esp_adc_cal_value_t r1;

    gpio_num_t adc1_gpio_num, adc_gpio_num, dac_gpio_num;

    r = adc2_pad_get_io_num( ADC2_EXAMPLE_CHANNEL, &adc_gpio_num );
    assert( r == ESP_OK );
    r = adc1_pad_get_io_num( ADC1_EXAMPLE_CHANNEL, &adc1_gpio_num );
    assert( r == ESP_OK );
    ...
    printf("ADC channel1 %d @ GPIO %d.\n", ADC1_EXAMPLE_CHANNEL, adc1_gpio_num );
    ...
    adc1_config_channel_atten(ADC1_EXAMPLE_CHANNEL,ADC_ATTEN_11db);

    // Calculate ADC characteristics i.e. gain and offset factors
    esp_adc_cal_characteristics_t characteristics;
    //esp_adc_cal_get_characteristics(V_REF, ADC_ATTEN_11db, ADC_WIDTH_12Bit, &characteristics);
    r1 = esp_adc_cal_characterize( ADC_UNIT_1, ADC_ATTEN_11db, ADC_WIDTH_12Bit, V_REF, &characteristics);
    if ( r1 == ESP_ADC_CAL_VAL_EFUSE_VREF ) {
        printf("eFuse Vref used for characterization\n");
    } else if ( r1 == ESP_ADC_CAL_VAL_EFUSE_TP ) {
        printf("Two Point value used for characterization (only in Linear Mode)");
    } else if ( r1 == ESP_ADC_CAL_VAL_DEFAULT_VREF ) {
    ...
    while(1) {
        ...
        //v = adc1_to_voltage( ADC1_EXAMPLE_CHANNEL, &characteristics );
        r = esp_adc_cal_get_voltage( ADC1_EXAMPLE_CHANNEL , &characteristics, &v);
        r = adc2_get_raw( ADC2_EXAMPLE_CHANNEL, ADC_WIDTH_12Bit, &read_raw);
        if ( r == ESP_OK ) {
            printf("%d: %d, %d mV\n", output_data, read_raw, v);
